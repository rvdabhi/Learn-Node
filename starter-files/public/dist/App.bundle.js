...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			...
			searchResult.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results found for ' + _this.value + ' found!</div>');
		}).catch(function (err) {
			console.error(err);
		});
	});
...
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 18.51;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 73.93;
